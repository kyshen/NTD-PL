...
\begin{figure}[htbp]
    ...
    \caption{线性一致性实验中的配对差值分布。图中比较不同设置下 Tucker 与 NTD-PL 的逐随机种子配对误差间隙，用于补充正文表~\ref{tab:linear-consistency} 的场景均值结论。可以看到，在 \texttt{bias=0} 时，不引入常数项的 NTD-PL 与 Tucker 的配对差值基本集中在零附近；而在存在仿射偏置时，$\beta_0$ 受限的模型与 Tucker 之间出现更明显的系统性间隙。}
...
\begin{figure}[htbp]
    ...
    \caption{固定二维 Tucker 局部 patch 时，不同多项式最高阶数 $P$ 下学习到的局部响应图与相对 $P=1$ 的偏差图。上排展示局部响应，下排展示相对线性基线的偏差。该图用于补充正文第~5.2.2 节关于“新增表达能力如何随阶数逐步进入局部响应结构”的讨论。}
...
\begin{figure}[htbp]
    ...
    \caption{CAVE 随机缺失补全的代表性空间可视化。各列依次为原图、带掩膜的观测输入、Tucker 补全、NTD-PL 补全、Tucker 的 missing-only 误差、NTD-PL 的 missing-only 误差，以及 missing-only 误差下降图。最后一列按 Tucker $-$ NTD-PL 定义，正值表示 NTD-PL 在缺失位置上进一步降低了补全误差。该图用于补充正文第~5.3.3 节关于“收益主要发生在缺失区域本身”的讨论。}
