...
\subsection{设计动机：从显式高阶交互到结构化压缩}

标准 Tucker 分解以共享核心张量和各模因子矩阵构成多线性生成机制。若记
\begin{equation}
\cS = \llbracket \cX;\mathbf A^{(1)},\dots,\mathbf A^{(N)}\rrbracket,
\end{equation}
则其本质是在低秩潜在空间中通过连续模乘生成观测张量。该结构在压缩、恢复与表示学习中具有良好的参数效率与结构透明性，但它默认观测条目可以由多线性机制充分刻画。对于存在饱和响应、非线性观测、复杂高阶耦合或曲面结构的数据，仅依赖多线性假设往往会带来系统性失配。
...
\end{equation}
其中每个模因子在第 $n$ 模上同时依赖 $p$ 个潜在索引。式~\eqref{eq:explicit_p_tucker_prototype} 说明了“高阶交互如何进入 Tucker”：它保留了逐模生成的 Tucker 外形，但把每一模的线性映射替换成了显式的 $p$ 阶局部耦合。对于 $p=2$，它对应二次 Tucker；一般 $p$ 时，可视为一个显式 $p$-Tucker 原型。下图用张量网络更直观地展示这种 Tucker 的高次扩展。
\begin{center}
...
基于这一观察，本文不直接训练式~\eqref{eq:explicit_p_tucker_prototype} 所对应的显式高阶模型，而是寻找一种结构化压缩：既保留“共享低秩 骨架 上存在高阶交互”的语义，又把参数规模重新压回 Tucker 级别。NTD-PL 正是这一设计目标下得到的模型。

\subsection{NTD-PL 模型定义与结构解释}

\subsubsection{最终模型定义}
...
\subsubsection{为什么这不是后处理}

NTD-PL 不能简单理解为先做一次 Tucker 重构，再对输出做逐元素校正。关键区别在于：式~\eqref{eq:ntdpl_model} 中的非线性作用对象不是一个与低秩结构无关的外部信号，而是共享 Tucker 潜在张量 $\cS$ 本身。由于 $\cS$ 的每个条目已经由核心张量与各模因子的多线性耦合生成，因此 $\cS^{\odot p}$ 对应的是同一 骨架 上交互模式的重组，而不是对一个已经完成的线性重构施加独立校准。后文第 5.4.4 节将专门通过 Tucker 的多项式后处理 与 NTD-PL 的对比，区分这两种机制。

\subsubsection{作为显式高阶 Tucker 的结构化压缩}

虽然 NTD-PL 的主角是式~\eqref{eq:ntdpl_model}，但它可以被理解为从显式高阶 Tucker 原型出发得到的一种结构化压缩。以二次情形为例，若将式~\eqref{eq:explicit_p_tucker_prototype} 中的模因子写成
\begin{equation}
 a^{(n)}_{i_n,r_n,r_n'} \approx b^{(n)}_{i_n,r_n}c^{(n)}_{i_n,r_n'},
\end{equation}
则二次项可近似重写为两个 Tucker 张量的逐元素乘积：
\begin{equation}
...
其中
\begin{equation}
\cS_B = \llbracket \cX;\mathbf B^{(1)},\dots,\mathbf B^{(N)}\rrbracket,
\qquad
\cS_C = \llbracket \cX;\mathbf C^{(1)},\dots,\mathbf C^{(N)}\rrbracket.
\end{equation}
...
\end{equation}
按同样思路推广到更高阶，就得到共享 骨架 上的 $\cS^{\odot p}$。于是，NTD-PL 可被理解为将这些共享投影的 $p$ 阶交互项按系数线性组合，从而把显式高阶 Tucker 的复杂性压缩到少量链接系数中。
\begin{center}
\resizebox{\linewidth}{!}{\input{./artifacts/p-tucker-hadamard.tex}}
\end{center}

\subsection{NTD-PL 的基本性质}

本节概括 NTD-PL 的三个最基本性质：模型族关系与参数效率、逐元素非线性生成下的表达能力，以及尺度不定性与归一化机制。详细证明见附录~\ref{app:model_properties}--\ref{app:expressivity}。这些性质分别对应后文的线性一致性验证、有限次多项式链路匹配实验与平滑非多项式逼近实验。
...
这两个命题说明，NTD-PL 构成了一族关于多项式次数 $P$ 的嵌套模型：当高阶项关闭时，它精确回到 Tucker；随着 $P$ 增加，模型表达力按次数受控扩张。这也是后文采用 degree continuation 的理论基础。
...
命题~\ref{prop:param_complexity} 表明，NTD-PL 的额外自由度只体现为 $P+1$ 个标量系数，而不是新的高阶因子张量或高阶核心。因而，相比显式 $p$-Tucker，NTD-PL 不是通过增加另一套高阶参数化来增强表达力，而是在基本保持 Tucker 级别参数规模的前提下引入受控非线性。这一点将在第 5.2 节和第 5.4 节的实验中得到直接体现。
...
\cS^\star
=
\llbracket \cX^\star;\mathbf A^{(1)\star},\dots,\mathbf A^{(N)\star}\rrbracket,
\end{equation}
...
命题~\ref{prop:poly_exact_main} 对应第 5.2.2 节的有限次多项式链路匹配实验；定理~\ref{thm:analytic_bound_main} 则解释了第 5.2.3 节中“随着 $P$ 增加，平滑非多项式数据可被系统逼近”的现象。两者共同表明，NTD-PL 的表达力并不是来自模糊的“增参”，而是来自共享 骨架 上可分析的逐元素非线性展开。
...
为了在同一框架下同时处理完整张量分解与缺失观测补全，记二值观测掩码张量仍为 $\Omega\in\{0,1\}^{I_1\times\cdots\times I_N}$。考虑如下目标函数：
\begin{equation}
...
\end{equation}
其中 $\cS=\llbracket \cX;\mathbf A^{(1)},\dots,\mathbf A^{(N)}\rrbracket$，且
\begin{equation}
Z_\Omega =
\begin{cases}
...
对完整观测使用标准 Tucker 初始化，对缺失观测使用 masked Tucker 初始化 \cite{yang2016iterative, liu2012tensor}。对多项式系数，采用线性 warm start：
\begin{equation}
\beta_0=0,\qquad \beta_1=1,\qquad \beta_p=0\quad (p\ge 2).
\label{eq:beta_warm_start}
\end{equation}
本文采用逐次延续的方式，训练从 $p=1$ 的线性情形开始，随着优化推进再逐步激活更高阶项，意味着模型总是从标准 Tucker 解附近出发，而不是从一个高阶非线性模型直接起步。
...
由于逐元素多项式链接打破了经典 Tucker 中最小二乘子问题的闭式结构，本文不采用 ALS，而是使用 Adam \cite{kingma2014adam} 对 $\cX$ 与各模因子做梯度更新。换言之，NTD-PL 在优化层面与标准 Tucker 的最本质区别之一就在于：低秩骨架被保留，但 entry model 的非线性使得经典 ALS 闭式子问题不再存在。

\subsubsection{多项式系数的 ridge 更新}
...
\paragraph{实现注记：标准化幂基上的稳定 $\boldsymbol\beta$ 更新.}
...
R=\prod_{n=1}^{N}R_n.
\end{equation}
这里 $M$ 表示完整张量的条目总数，$M_\Omega$ 表示观测条目数。需要强调的是，\textbf{当前实现采用的是稠密 Tucker 重构与稠密多项式前向/反向计算}：即便在补全任务中，潜在张量 $\cS$、预测张量 $\widehat{\cY}$ 以及 $f'_{\boldsymbol\beta}(\cS)$ 仍是在完整张量上构造，只是在残差处通过掩码筛选观测项。因此，单轮外迭代的主导成本依然主要随 $M$ 缩放；只有 $\boldsymbol\beta$ 的 ridge 子问题明确只依赖 $M_\Omega$。

\paragraph{Tucker 潜在张量构造.}
...
\paragraph{$\boldsymbol\beta$ 的 ridge 更新.}
...
\end{equation}
因此，NTD-PL 相比标准 Tucker 的额外计算主要来自两部分：其一是逐元素多项式链接及其导数的计算，其代价为低阶、可并行的 $\mathcal O(PM)$；其二是 $\boldsymbol\beta$ 的小规模 ridge 更新，其代价为 $\mathcal O(PM_\Omega + M_\Omega P^2 + P^3)$。换言之，当前实现下的主导成本仍然来自 Tucker 骨架本身的前向/反向传播，而不是多项式链接系数的求解。这也解释了后文第 5.3 节中观察到的现象：NTD-PL 的 wall-clock 增量是可量化的，但其主要来源并非 $\boldsymbol\beta$ 子问题，而是围绕共享 骨架 的非线性前向/反向计算。
