...
\subsection{Proof of Proposition~\ref{paper-prop:shared_pfold}}

Substitute \eqref{paper-eq:shared_pfold_factors} into
\eqref{paper-eq:pfold_tucker}. The products can be regrouped as
\[
\begin{aligned}
  ...
  \sum_{\mathbf r^{(\ell)}}
  X_{\mathbf r^{(\ell)}}
  \prod_{n=1}^{N}A^{(n)}_{i_n,r_n^{(\ell)}}
  \right) \\
  &= S_{\mathbf i}^{p}.
\end{aligned}
\]
Applying this identity entrywise gives
$\mathcal{T}_p(\cX)=\cS^{\od p}$.

\subsection{Proof of Proposition~\ref{paper-prop:rank_inflation}}
...
\subsection{Proof of Theorem~\ref{paper-thm:tucker_separation}}
...
represented by a degree-\(p\) NTD-PL model using the rank-\((R,\ldots,R)\)
Tucker signal.

\subsection{Proof of Corollary~\ref{paper-cor:tied_lift_parameters}}

The direct parameterization in \eqref{paper-eq:pfold_tucker} stores
$I_nR_n^p$ entries in the mode-$n$ map. By
Proposition~\ref{paper-prop:rank_inflation}, the shared degree-$p$ term can also
be written as a Tucker tensor with ranks at most
...
contrast, NTD-PL with the power basis reuses the core and factors of
$\cS$ and adds only one response coefficient per polynomial term. Summing degrees up to
$P$ adds $P+1$ response coefficients, proving the stated comparison.

\subsection{Proof of Proposition~\ref{paper-prop:poly_generation}}

If $g(s)=\sum_{p=0}^{P}\gamma_p s^p$ and $\cY=g(\cS^\star)$, choose the
Tucker component of NTD-PL to be the Tucker representation of
...
\subsection{Proof of Theorem~\ref{paper-thm:response_approximation}}

By the Weierstrass approximation theorem, for every $\epsilon>0$ there exists
a polynomial $\pi_P(s)=\sum_{p=0}^{P}\beta_ps^p$ such that
$\sup_{s\in[a,b]}|f(s)-\pi_P(s)|\le\epsilon$. Every entry of $\cS^\star$ lies in
$[a,b]$, so each entrywise approximation error is bounded by $\epsilon$.
Therefore
\[
  \norm{f(\cS^\star)-\pi_P(\cS^\star)}_F^2
  =
  \sum_{\mathbf{i}}
  |f(S^\star_{\mathbf{i}})-\pi_P(S^\star_{\mathbf{i}})|^2
  \le
  M\epsilon^2,
\]
which gives the stated bound after taking square roots.

\subsection{Proof of Proposition~\ref{paper-prop:monotone_descent}}

For a fixed Tucker signal, Response Refresh solves the $\bbeta$-subproblem
exactly, hence it cannot increase $\mathcal{F}_J$. The subsequent Tucker
update is assumed to satisfy the sufficient decrease condition
\eqref{paper-eq:sufficient_decrease}, and therefore also cannot increase
$\mathcal{F}_J$. Combining the two stages gives monotone decrease over the
outer iteration. Since $\mathcal{F}_J$ is bounded below by zero when the
regularization weights are nonnegative, summing
\eqref{paper-eq:sufficient_decrease} over $k$ gives
\[
  c\sum_{k=0}^{K}\norm{\theta^{k+1}-\theta^k}_2^2
  \le
  \mathcal{F}_J(\theta^0)-\mathcal{F}_J(\theta^{K+1})
  \le
  \mathcal{F}_J(\theta^0).
\]
Letting $K\to\infty$ proves square summability of the steps.

\subsection{Proof of Theorem~\ref{paper-thm:stationary_convergence}}

For fixed $J$, the objective is a polynomial function of the Tucker parameters
...
change of coefficients. The linear-spline basis is piecewise polynomial and
semi-algebraic; standard RBF bases are analytic and belong to the
...
property. Assumption~\ref{paper-ass:optimization} gives sufficient decrease and a
...
critical point of $\mathcal{F}_J$.

\subsection{Proof of Theorem~\ref{thm:supp-masked-generalization}}
