...
\begin{table}[ht]
  \centering
  \caption{Link Refresh diagnostic on CAVE completion. Low/mid/high groups are
  \(D_{\mathrm{link}}\) terciles, and gains report mean NTD-PL RMSE$^\ast$
  improvement over matched-rank Tucker within each tercile.}
...
\begin{figure}[H]
  ...
  \caption{CAVE beads visual comparison. Rows show full reconstruction and
  random completion at missing rate \(0.5\). Error maps show Tucker minus
...
\begin{figure}[ht]
  ...
  \caption{Per-unit non-HSI diagnostic correlation. \(D_{\mathrm{link}}\)
  from frozen-backbone Link Refresh is plotted against NTD-PL RMSE gain for
  natural-image, object-view, and action-video units; labels are assigned from
  \(D_{\mathrm{link}}\), not from NTD-PL gain.}
