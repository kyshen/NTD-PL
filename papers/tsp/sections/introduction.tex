...
coupled low-dimensional factors. They have become standard tools for signal representation,
...
the measured tensor. This amounts to assuming that the map from the underlying
...
between the underlying physical quantity and the stored measurement
...
response acts entrywise on an underlying low-rank tensor, the measured tensor need
not remain low-rank in the same Tucker sense. An underlying low-rank signal can
...
the rank budget selected for the underlying signal may be appropriate, while the
...
rank-expansion mechanism explicit. The model is therefore not an unrestricted
...
the response creates rank expansion tied to the same Tucker factors. The degree-$p$ term
...
We also introduce a frozen-Tucker diagnostic that measures how much residual
...
side, we characterize when an entrywise response creates efficient rank expansion and
...
\begin{itemize}
  ...
  hyperspectral recovery tasks, with rank-expansion comparisons,
