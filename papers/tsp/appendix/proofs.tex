...
\subsection{Proof of Proposition~\ref{prop:rank_expansion}}
...
\]
Expanding $S_{\mathbf{i}}^p$ gives products of $p$ latent Tucker tuples. For
...
CP-diagonal Tucker backbone
\[
  \cS =
  \sum_{r=1}^{R} a_r^{(1)}\otimes\cdots\otimes a_r^{(N)} .
\]
For generic factors, each mode unfolding of \(\cS\) has rank \(R\), so the
backbone has exact Tucker rank \((R,\ldots,R)\).

Now set the degree-\(p\) link coefficient to one and all other coefficients to
...
By Proposition~\ref{prop:rank_expansion}, the degree-$p$ channel can be written
...
contrast, the power-dictionary linked model reuses the latent core and factors of
$\cS$ and adds only one scalar coefficient per channel. Summing degrees up to
$Q$ adds $Q+1$ link coefficients, proving the stated comparison.

\subsection{Proof of Theorem~\ref{thm:poly_generation}}

If $g(s)=\sum_{p=0}^{P}\gamma_p s^p$ and $\cY=g(\cS^\star)$, choose the Tucker
backbone of the power-dictionary linked model to be the Tucker representation of
...
For a fixed Tucker backbone, Link Refresh solves the $\bbeta$-subproblem
exactly, hence it cannot increase $\mathcal{F}_Q$. The subsequent backbone
...
and link coefficients when the monomial basis is used. For any other fixed
